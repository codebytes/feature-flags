# Feature Flags: The Art of the IF and Deployment

Explore feature flags, their uses, operational practices, and best practices with slides and hands-on demos.

## Slides

[View HTML slides](https://chris-ayers.com/feature-flags/) | [Download PDF](https://chris-ayers.com/feature-flags/Slides.pdf)

## Resources

- [Slide source](slides/Slides.md)
- Demos: [Feature Management](feature-management/), [Azure App Configuration](appconfig-sample/), and [LaunchDarkly](launch-darkly-sample/)
- [Microsoft Feature Management](https://github.com/microsoft/FeatureManagement-Dotnet)
- [Azure App Configuration](https://docs.microsoft.com/en-us/azure/azure-app-configuration/)
- [OpenFeature](https://openfeature.dev/)

## Contact

**Chris Ayers**, Principal Software Engineer

[Blog](https://chris-ayers.com/) | [GitHub](https://github.com/codebytes) | [LinkedIn](https://linkedin.com/in/chris-l-ayers/) | [Bluesky](https://bsky.app/profile/chris-ayers.com) | [Mastodon](https://hachyderm.io/@Chrisayers) | [Twitter](https://twitter.com/Chris_L_Ayers)

## License

This project is licensed under the MIT License. See the [LICENSE](LICENSE) file for more information.